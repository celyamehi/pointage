-- Vue matérialisée simplifiée pour le suivi mensuel des absences ET retards
-- Sans les montants déduits - juste les absences (jours) et retards (minutes)

CREATE MATERIALIZED VIEW IF NOT EXISTS absences_retards_simplifies_agents AS
WITH 
-- Paramètres fixes (heures de référence uniquement)
params AS (
    SELECT 
...
pointages_agent AS (
    ...
    GROUP BY td.date_jour, td.debut_mois, td.fin_mois, a.id, a.nom, a.email, a.role, td.annee, td.mois, td.periode
),
...
calculs_quotidiens AS (
    SELECT 
        ...
        -- Absence partielle = absent une seule session (XOR remplacé par logique)
        CASE WHEN ((po.matin_arrivee IS NULL AND po.matin_sortie IS NULL) AND 
                   (po.apres_midi_arrivee IS NOT NULL OR po.apres_midi_sortie IS NOT NULL))
              OR ((po.apres_midi_arrivee IS NULL AND po.apres_midi_sortie IS NULL) AND 
                   (po.matin_arrivee IS NOT NULL OR po.matin_sortie IS NOT NULL)) THEN true ELSE false END as absent_partiel
...
totaux_mensuels AS (
    SELECT 
        ...
        mois,
        periode,
        MIN(debut_mois)::date as debut_mois,
        MAX(fin_mois)::date as fin_mois,
...
SELECT 
    tm.*,
    -- Indicateurs de performance (sans montants)
    CASE 
    ...
    TO_CHAR(tm.debut_mois, 'Month YYYY') as mois_formaté,
...
CREATE INDEX IF NOT EXISTS idx_absences_retards_simpl_agent_periode ON absences_retards_simplifies_agents(agent_id, periode);
CREATE INDEX IF NOT EXISTS idx_absences_retards_simpl_periode ON absences_retards_simplifies_agents(periode);
CREATE INDEX IF NOT EXISTS idx_absences_retards_simpl_agent_annee_mois ON absences_retards_simplifies_agents(agent_id, annee, mois);

-- COMMENTAIRE: Pour mettre à jour la vue matérialisée, exécutez:
-- REFRESH MATERIALIZED VIEW absences_retards_simplifies_agents;
