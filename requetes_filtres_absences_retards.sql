-- Requêtes pour filtrer les absences ET retards par période
-- À utiliser après avoir créé la vue matérialisée absences_retards_mensuels_agents
...
SELECT 
    ...
    montant_total_deduit,
    performance_globale,
    dates_absences_completes,
    dates_retards
FROM absences_retards_mensuels_agents 
...
SELECT 
    ...
    SUM(montant_total_deduit) as montant_total_deduit_periode,
...
SELECT 
    ...
    SUM(montant_total_deduit) as montant_total_deduit_mois
FROM absences_retards_mensuels_agents 
...
-- 10. Rapport détaillé pour export Excel
SELECT 
    agent_nom,
    agent_email,
    agent_role,
...
SELECT 
    ...
    ROUND(AVG(montant_total_deduit), 2) as moyenne_montant_deduit
FROM absences_retards_mensuels_agents 
WHERE periode = TO_CHAR(CURRENT_DATE, 'YYYY-MM')
...
SELECT 
    ...
    COALESCE(current_month.montant_total_deduit, 0) as montant_mois_en_cours,
    COALESCE(previous_month.montant_total_deduit, 0) as montant_mois_precedent
FROM agents a
LEFT JOIN absences_retards_mensuels_agents current_month ON a.id = current_month.agent_id 
    AND current_month.periode = TO_CHAR(CURRENT_DATE, 'YYYY-MM')
LEFT JOIN absences_retards_mensuels_agents previous_month ON a.id = previous_month.agent_id 
